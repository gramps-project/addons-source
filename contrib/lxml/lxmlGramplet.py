...
class lxmlGramplet(Gramplet):
        ...
        for one in root.getchildren():
            ...
            for two in one.getchildren():
                ...
                for three in two.getchildren():
                    ...
                    if three.tag == '{http://gramps-project.org/xml/1.5.0/}ptitle':
                        places.append(three.text)
                    if three.tag == '{http://gramps-project.org/xml/1.5.0/}stitle':
    ...
    def WriteXML(self, log, first, last, surnames, places, sources):
        ...
        xml.set("date", gen.datehandler.displayer.display(time))
