"""Linux-only unit tests for libtmg.py

Split off from test_libtmg.py: every class in this module creates an
in-memory Gramps SQLite database via

    make_database("sqlite").load(":memory:", None)

which currently hangs on Windows under the conda-forge GTK + pip Gramps
combination used by the CI unit-test-windows job. Pure-logic tests that
do not touch the Gramps DB layer stay in test_libtmg.py and run on every
OS.

Filename convention (see .github/workflows/ci.yml):
...
# ---------------------------------------------------------------------------
# import_notes — per-person note creation from tmg events
# ---------------------------------------------------------------------------
