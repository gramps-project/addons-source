...
class Filter(Gramplet):
    ...
    def __filter_clicked(self):
        ...
        """
        # Guard against re-entrant clicks (e.g. pressing Enter again before
        # the previous build_tree() has finished), which can otherwise hit
        # the person tree view mid-rebuild.
        if self._building:
            return
        self._building = True
        try:
