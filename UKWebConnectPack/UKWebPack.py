...
WEBSITES = [
    ...
    ["Person", "FamilySearch", _("FamilySearch.org"), "https://www.familysearch.org/search/record/results?q.birthLikeDate.from=%(birth)s&q.birthLikeDate.to=%(birth)s&q.deathLikeDate.from=%(death)s&q.deathLikeDate.to=%(death)s&q.givenName=%(given)s+%(middle)s&q.surname=%(surname)s"], #Free Account Needed
