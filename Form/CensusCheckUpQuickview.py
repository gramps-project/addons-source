...
def run(database, document, person):
    ...
    ancestor_list = []
    ancestor = sdb.father(person)
    if ancestor not in ancestor_list:
        ancestor_list.append(ancestor)
    ancestor = sdb.mother(person)
    if ancestor not in ancestor_list:
        ancestor_list.append(ancestor)

    for family in sdb.parent_in(person):
        father = sdb.father(family)
        mother = sdb.mother(family)
        ...
        ancestor = sdb.father(spouse)
        if ancestor not in ancestor_list:
            ancestor_list.append(ancestor)
        ancestor = sdb.mother(spouse)
        if ancestor not in ancestor_list:
            ancestor_list.append(ancestor)

    for ancestor in ancestor_list:
        process_person(database, sdb, stab, ancestor, census_list)
