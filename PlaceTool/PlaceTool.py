...
class PlaceTool(Gramplet):
    ...
    def cb__apply(self,obj):
        with DbTxn(_("Setting place properties"), self.dbstate.db) as self.trans:
            ...
            for handle in selected_handles:
                ...
                if typename: self.__set_type(place,typename)
                if tag: self.__set_tag(place,tag)
    ...
    def get_level(self, handle):
        level = {}
        for obj, h in self.dbstate.db.find_backlink_handles(handle, ['Place']):
            place = self.dbstate.db.get_place_from_handle(h)
            level[place.get_name().get_value()] = (h,place)
        return level
