...
"""
A citation formatter that that adds a repoository and call number to the
...
class Formatter:
    ...
    def format(self, bibliography, db, elocale):
        ...
        for citation in bibliography.get_citation_list():
            ...
            src = _format_source_text(db, source, elocale)
...
def _format_source_text(db, source, elocale):
    if not source:
        return ""
    ...
    if source.get_reporef_list():
        reporef = source.get_reporef_list()[0]
        repo = db.get_repository_from_handle(reporef.ref)
        repo_name = repo.get_name()
        call_number = reporef.get_call_number()
        if call_number:
            src_txt += trans_text("{repo_name} [{call_number}]").format(
                repo_name=repo_name, call_number=call_number
            )
        else:
            src_txt += repo_name
