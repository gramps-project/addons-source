                    ...
                    for handle in assoc.get_note_list():
                        ...
                        for line in note.get().split('\n'):
                            ...
                            if data[0] == -1:
                                ...
                                side = data[2][0].upper()
                            cms = float(field[4])
                            segmap.segments.append([chromo, start, stop, side, cms, _nd.display(associate), rgb_color])
...
class SegmentMap(Gtk.DrawingArea):
    ...
    def do_draw(self, cr):
        ...
        cr.set_line_width(1)
        self.__rects = []
        for chromo, start, stop, side, cms, assoc_name, rgb_color in self.segments:
            try:
    ...
    def on_pointer_motion(self, _dummy, event):
        ...
        if self.__active != active:
            ...
            if active == -1:
                ...
                self.set_tooltip_text("{0}\n{1} cMs".format(self.segments[active][5], self.segments[active][4]))
