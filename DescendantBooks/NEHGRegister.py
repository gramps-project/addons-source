...
"""
Reports/Books/New England Historical and Geanealogical Register report
"""
...
class NEHGRegister():
    def __init__(self, dbstate, uistate):
        RunReport(dbstate, uistate, "NEHGRegisterReport",
            "detailed_descendant_book", "Detailed Descendant Book",
            "NEHGRegisterReport", "NEHGRegisterOptions")

class NEHGRegisterReport(Report):

    def __init__(self, database, options, user):
        """
        Create the NEHGRegister object that produces the report.
...
#BOOK end
    ...
    def __write_children(self, family):
        ...
        father_name, mother_name = self.__get_mate_names(family)

        self.doc.start_paragraph("DDR-ChildTitle")
        self.doc.write_text(
                        self._("Known children of %(father_name)s and %(mother_name)s") %
...
#
#------------------------------------------------------------------------
class NEHGRegisterOptions(MenuReportOptions):
    """
    ...
    def add_menu_options(self, menu):
        ...
        self.__filter = FilterOption(_("Filter"), 1)
        ...
        numbering = EnumeratedListOption(_("Numbering system"), "Record (Modified Register)")
        numbering.set_items([
        ...
        generations = NumberOption(_("Generations"), 3, 1, 100)
        ...
        computeage = BooleanOption(_("Compute death age"),False)
        ...
        verbose = BooleanOption(_("Use complete sentences"), False)
        verbose.set_help(
        ...
        incnotes = BooleanOption(_("Include notes"), False)
        ...
        incappearances = BooleanOption(_("Include the 'Report appearances' "
                                         "section with each person"), False)
    ...
    def make_default_style(self, default_style):
        ...
        font = FontStyle()
        font.set(face=FONT_SERIF, size=14, italic=1)
        para = ParagraphStyle()
        para.set_font(font)
        ...
        para.set_top_margin(0.00)
        para.set_bottom_margin(0.00)
        ...
        font = FontStyle()
        font.set(size=10, face=FONT_SERIF, bold=1)
        para = ParagraphStyle()
        para.set_font(font)
        para.set(first_indent=0.0, lmargin=1.5)
        para.set_top_margin(0.25)
        ...
        font = FontStyle()
        font.set(face=FONT_SERIF, size=12)
        para = ParagraphStyle()
        para.set_font(font)
        para.set(first_indent=0.0, lmargin=1.5)
        para.set_top_margin(0.25)
        ...
        font = FontStyle()
        font.set(face=FONT_SERIF, size=12, italic=0)
        para = ParagraphStyle()
        para.set_font(font)
        para.set_top_margin(0.25)
        para.set_bottom_margin(0.25)
