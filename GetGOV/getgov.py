...
class GetGOV(Gramplet):
        ...
        with DbTxn(_("Add GOV-id place %s") % gov_id, self.dbstate.db) as trans:
            while to_do:
                ...
                if place is not None:
                    ...
                    place, ref_list = self.__get_place(
        ...
        if len(ptype):
            ...
            if len(value):
                ...
                if tuple([type_code, curr_lang]) in type_dic:
                    place_type.set_from_xml_str(
                        type_dic.get(tuple([type_code, curr_lang]), "No Type")
                    )
                elif tuple([type_code, preferred_lang]) in type_dic:
                    place_type.set_from_xml_str(
                        type_dic.get(tuple([type_code, preferred_lang]), "No Type")
                    )
                elif tuple([type_code, "de"]) in type_dic:
                    place_type.set_from_xml_str(
                        type_dic.get(tuple([type_code, "de"]), "No Type")
                    )
                elif tuple([type_code, "en"]) in type_dic:
                    place_type.set_from_xml_str(
                        type_dic.get(tuple([type_code, "en"]), "No Type")
                    )
        return place_type
    ...
    def __get_date_range(self, element):
        ...
        if begin_str and end_str:
            date_str = _("from %(begin)s to %(end)s" % {'begin': begin_str, 'end': end_str} )
        elif begin_str:
