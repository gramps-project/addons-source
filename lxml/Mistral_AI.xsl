...
<xsl:for-each select="query/surnames/surname">
    ...
    <xsl:text>{"role":"user","content":"User interaction n°</xsl:text>
        <xsl:value-of select="$spos"/>
    <xsl:text> contained in document n°</xsl:text>
        <xsl:value-of select="$spos"/>
    <xsl:text> with surname </xsl:text>
       <xsl:value-of select="."/>
    <xsl:text>"}</xsl:text>
    <xsl:text>{"role":"assistant","content":"Bot interaction n°</xsl:text>
        <xsl:value-of select="$spos"/>
    <xsl:text> contained in document n°</xsl:text>
        <xsl:value-of select="$spos"/>
    <xsl:text> with surname </xsl:text>
...
<xsl:for-each select="query/sources/source">
    <xsl:variable name="scpos">
       <xsl:number value="position()" format="1"/>
    </xsl:variable>
    <xsl:text>{"messages":[</xsl:text>
    <xsl:text>{"role":"user","content":"User interaction n°</xsl:text>
        <xsl:value-of select="$scpos"/>
    <xsl:text> contained in document n°</xsl:text>
        <xsl:value-of select="$scpos"/>
